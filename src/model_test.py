...
#         self.W_in = None
#         self.W_out = None
#         self.N = None  # N-gram embedding matrix
...
#         self.min_n = 3  # Minimum n-gram length
#         self.max_n = 6  # Maximum n-gram length
#         self.ngram2idx = {}  # Dict: n-gram -> index
#         self.idx2ngram = {}  # Dict: index -> n-gram
#         self.ngram_vocab_size = 0
#         self.word_ngrams = {}  # Cache: word_idx -> list of ngram indices
...
#             self.word_ngrams[word_idx] = ngram_indices
        
#         print(f"Cached n-grams for {len(self.word_ngrams)} words")
...
#                 ngram = extended_word[i:i+n]
#                 ngrams.append(ngram)

#         return ngrams

#     def build_ngram_vocab(self, all_tokens):
#         ngram_set = set()

#         for words in set(all_tokens):
#             ngram = self.get_ngrams(words)
#             ngram_set.update(ngram)

#         ngram_list = sorted(list(ngram_set))
#         self.ngram2idx = {ngram: idx for idx, ngram in enumerate(ngram_list)}
#         self.idx2ngram = {idx: ngram for ngram, idx in self.ngram2idx.items()}
#         self.ngram_vocab_size = len(ngram_list)

#         print(f"N-gram vocabulary built: {self.ngram_vocab_size} unique n-grams")
...
#         self.build_ngram_vocab(all_tokens)
#         self.cache_word_ngrams()
...
#         self.N = nn.Embedding(self.ngram_vocab_size, self.embedding_dim)
...
#         self.N.weight.data.uniform_(-init_range, init_range)
                
#         # Move everything to GPU/CPU at once
#         self.to(self.device)
        
#         print(f"Embeddings initialized: {self.vocab_size} x {self.embedding_dim}")
#         print(f"N-gram embeddings initialized: {self.ngram_vocab_size} x {self.embedding_dim}")
...
#     def get_input_embedding(self, word_indices):
#         # Dim of word_indices: (batch,)
#         # Get word embeddings: (batch, emb_dim)
#         word_embeds = self.W_in(word_indices)
        
#         # Get n-gram embeddings for each word in the batch
#         batch_size = word_indices.size(0)
#         ngram_embeds_list = []
        
#         for i in range(batch_size):
#             word_idx = word_indices[i].item()
#             # Get cached n-gram indices for this word
#             ngram_indices = self.word_ngrams.get(word_idx, [])
            
#             if len(ngram_indices) > 0:
#                 # Convert to tensor and get embeddings
#                 ngram_indices_tensor = torch.LongTensor(ngram_indices).to(self.device)
#                 ngram_embeds = self.N(ngram_indices_tensor)  # (num_ngrams, emb_dim)
#                 # Average all n-gram embeddings for this word
#                 avg_ngram_embed = ngram_embeds.mean(dim=0)  # (emb_dim,)
#             else:
#                 # No n-grams (e.g., for <UNK>, <PAD>), use zero embedding
#                 avg_ngram_embed = torch.zeros(self.embedding_dim, device=self.device)
            
#             ngram_embeds_list.append(avg_ngram_embed)
        
#         # Stack all n-gram embeddings: (batch, emb_dim)
#         ngram_embeds = torch.stack(ngram_embeds_list, dim=0)
        
#         # Combine word embeddings and n-gram embeddings
#         # Dim of input_embeds: (batch, emb_dim)
#         input_embeds = word_embeds + ngram_embeds

#         return input_embeds

#     def loss_function(self, target_list, context_list, negative_samples):
#         # Get input embeddings (word + n-grams)
#         # Dim of input_embeds: (batch, emb_dim)
#         input_embeds = self.get_input_embedding(target_list)

#         # Positive samples (actual context words)
#         # Dim of positive_out: (batch, emb_dim)
#         positive_out = self.W_out(context_list)
#         # Dim of positive_score: (batch,)
#         positive_score = torch.sum(input_embeds * positive_out, dim=1)

#         # Negative samples
#         # Dim of negative_out: (batch, neg_samples, emb_dim)
#         negative_out = self.W_out(negative_samples)
#         # Dim of negative_score: (batch, neg_samples)
#         negative_score = torch.bmm(negative_out, input_embeds.unsqueeze(-1)).squeeze(-1)

#         # Negative log-likelihood with sigmoid
#         # Maximize log(sigmoid(positive_score)) and log(sigmoid(-negative_score))
